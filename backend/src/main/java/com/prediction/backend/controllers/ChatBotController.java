...
public class ChatBotController {
    ...
    @PostMapping("/message")
    public ApiResponse<BotResponse> chatWithBot(
        ...
        botMessage.setContent(chatService.handleData(userMessage.getContent(), userMessage.getConversationId()).block());
        ...
        return ApiResponse.<BotResponse>builder()
                .message("Bot response sucessfully!")
                .data(botResponse)
                .build();
    }
    ...
    public Mono<ResponseEntity<ApiResponse<String>>> ask(
            @RequestParam("userId") String conversationId,
            @RequestParam("message") String message) {

        return chatService.handleData(message, conversationId)
