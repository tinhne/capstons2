    ...
    */
    public static boolean isValidMedicalJsonFormat(String jsonString) {
        try {
            ...
            // Check metadata
            if (!obj.has("metadata") || !obj.get("metadata").isJsonObject()) return false;
            JsonObject metadata = obj.getAsJsonObject("metadata");
            if (!metadata.has("age") || !metadata.has("gender") || !metadata.has("region")) return false;

            if (!metadata.get("age").isJsonPrimitive() || !metadata.get("age").getAsJsonPrimitive().isNumber()) return false;

            String gender = metadata.get("gender").getAsString();
            if (!List.of("Male", "Female", "Other").contains(gender)) return false;

            if (!metadata.get("region").isJsonPrimitive() || !metadata.get("region").getAsJsonPrimitive().isString()) return false;

            // Check symptoms
            if (!obj.has("symptoms") || !obj.get("symptoms").isJsonArray()) return false;
            JsonArray symptoms = obj.getAsJsonArray("symptoms");

            for (int i = 0; i < symptoms.size(); i++) {
                JsonObject sym = symptoms.get(i).getAsJsonObject();
                if (!sym.has("name") || !sym.has("level")) return false;

                String level = sym.get("level").getAsString();
                if (!List.of("mild", "moderate", "severe").contains(level)) return false;
            }
